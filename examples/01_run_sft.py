# minitune/examples/01_run_sft.py
# $ accelerate launch --gpu_ids 0,1,2,3 --config_file configs/fsdp_a40.yaml examples/01_run_sft.py
import typer
import torch
import math
...
def compute_metrics(trainer):
    ...
    # Use a small subset for speed during training (e.g., first 50 items)
    # We create a temporary dataloader just for this check
    eval_subset = trainer.eval_dataset.select(range(min(50, len(trainer.eval_dataset))))
    
    collate_fn = DataCollatorForLanguageModeling(trainer.tokenizer, mlm=False)
    loader = DataLoader(eval_subset, batch_size=trainer.config.sft.batch_size, collate_fn=collate_fn)
    loader = trainer.accelerator.prepare(loader) # Prepare for DDP
